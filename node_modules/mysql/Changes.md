...
* Re-use connection from pool after `connection.changeUser` is used #837 #1088
...
* Fix domain binding with `connection.connect`
...
* Return `Query` object from `pool.query` like `connection.query` #830
* Use `EventEmitter.listenerCount` when possible for faster counting

## v2.3.2 (2014-05-29)

* Fix pool leaking connections after `connection.changeUser` #833

## v2.3.1 (2014-05-26)

* Add database errors to error constants
* Add global errors to error constants
* Throw when calling `connection.release` multiple times #824 #827
...
* Remove connection from pool after `connection.changeUser` is released #806
...
* Make callback to `pool.query` optional like `connection.query` #585
...
* fix specifying `values` in `connection.query({...}).on(...)` pattern #755
